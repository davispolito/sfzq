function(_juce_find_module_dependencies module_header module_deps)
...
endfunction(_juce_find_module_dependencies)


function(_sfzero_load_module module)
    message(STATUS "Loading module: ${module}")

    find_path(sfzero_module_path
            NAMES "${module}.h"
            PATHS "${SFZERO_MODULES_DIR}" "${SFZERO_MODULES_DIR}" "${SFZERO_MODULES_DIR}"
            PATH_SUFFIXES "${module}"
            REQUIRED
    )

    get_filename_component(module_parent_path ${sfzero_module_path} DIRECTORY)
    ...
    if(EXISTS "${sfzero_module_path}/${module}.cpp")
        message(STATUS "Adding source file: ${sfzero_module_path}/${module}.cpp")
        target_sources(${lib_name} PRIVATE "${sfzero_module_path}/${module}.cpp")
    endif()

    # Load any modules that this one depends on:
    _sfzero_find_module_dependencies(${sfzero_module_path}/${module}.h module_dependencies)
    unset(sfzero_module_path CACHE)
    foreach(module_dep IN LISTS module_dependencies)
        ...
        _sfzero_load_module(${module_dep})
    endforeach()
endfunction(_sfzero_load_module)

function(setup_sfzero_lib lib_name)
    set(multiValueArgs MODULES)
    cmake_parse_arguments(CHOWDSPLIB "" "" "${multiValueArgs}" ${ARGN})

    message(STATUS "Setting up sfzero static Lib: ${lib_name}, with modules: ${CHOWDSPLIB_MODULES}")
